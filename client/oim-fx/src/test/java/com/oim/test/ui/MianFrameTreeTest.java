 ...
 * @author Only
 */
public class MianFrameTreeTest extends Application {
	...
	private void initUserList() {
		...
		TreeNodePane rootPane = new TreeNodePane();
		TreeItem<Object> root = new TreeItem<Object>(rootPane);
		...
		for (int j = 0; j < 5; j++) {
			...
			TreeNodePane node = new TreeNodePane();
			TreeItem<Object> treeNode = new TreeItem<Object>(node);
			...
			for (int i = 0; i < 5; i++) {
				...
				TreeItem<Object> headNode = new TreeItem<Object>(head);
				treeNode.getChildren().add(headNode);
			}
			root.getChildren().add(treeNode);
		}
		
		TreeNodePane t = new TreeNodePane();
		TreeItem<Object> tNode = new TreeItem<Object>(t);
		t.setText("部门");
		t.getStyleClass().remove("list-top-title");
		...
		for (int i = 0; i < 5; i++) {
			...
			TreeItem<Object> headNode = new TreeItem<Object>(head);
			tNode.getChildren().add(headNode);
		}
